...
const PARENT_LINKS_CSV_TEMPLATE = [
  'parentEmail,parentName,scoutFirstName,scoutLastName,relationshipType',
  'parent@example.com,Mary Doe,Jane,Doe,mother',
  'dad@example.com,John Smith,Tim,Smith,father',
].join('\n');

export default function ImportParentLinksForm() {
  ...
  const downloadTemplate = () => {
    const blob = new Blob([`${PARENT_LINKS_CSV_TEMPLATE}\n`], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = 'parent-links-import-template.csv';
    anchor.click();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await adminBulkService.importParentLinks(file);
  ...
  return (
    <Card className="bg-white/95 shadow-md">
      <CardHeader>
        <CardTitle>Parent-Scout Link Import</CardTitle>
        <CardDescription>
          Establish approved parent-scout relationships in bulk. Scouts must already exist. New
          parent accounts receive a 72-hour invite link.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="space-y-2">
            <Input
              id="parent-links-import-file"
              type="file"
              accept=".csv,text/csv"
              onChange={(event) => setFile(event.target.files?.[0] ?? null)}
            />
            <p className="text-sm text-muted-foreground">
              Expected columns: parentEmail, parentName, scoutFirstName, scoutLastName,
              relationshipType (optional, defaults to &quot;guardian&quot;).
            </p>
            ...
            <div className="rounded-md border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">
              <div className="mb-2 flex items-center justify-between">
                <span className="font-medium">
                  {inviteLinks.length} new parent account{inviteLinks.length !== 1 ? 's' : ''}{' '}
                  created
                </span>
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? 'Starting import...' : 'Import Parent Links'}
          </Button>
        </form>
      </CardContent>
    </Card>
